...
exports.serializeParameterObject = (from, parameterObject) => pipeable_1.pipe(exports.getParameterObjectSchema(parameterObject), fp_ts_1.either.chain(schema => {
    if (reference_object_1.ReferenceObjectCodec.is(schema)) {
        ...
        return pipeable_1.pipe(schema, schema_object_1.serializeSchemaObject(from));
    }
...
exports.serializeQueryParameterToTemplate = (from, parameter, parameterSchema, serializedSchema, target) => {
    ...
    const encoded = required ? serialized_fragment_1.serializedFragment(`${serializedSchema.io}.encode(${target}['${parameter.name}'])`, serializedSchema.dependencies, serializedSchema.refs) :
        serialized_fragment_1.serializedFragment(`${serializedSchema.io}.encode(${target}['${parameter.name}'] || none)`, [...serializedSchema.dependencies, serialized_dependency_1.serializedDependency('none', 'fp-ts/lib/Option')], serializedSchema.refs);
