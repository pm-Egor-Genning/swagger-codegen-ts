...
exports.getParameters = reader_utils_1.combineReader(Reader_1.ask(), e => (from, operation, pathItem) => {
    ...
    for (const parameter of parameters) {
        ...
        const serialized = reference_object_1.ReferenceObjectCodec.is(parameter)
            ...
            : parameter_object_1.serializeParameterObject(from, resolved.right);
    ...
    if (Option_1.isSome(operation.requestBody)) {
        ...
        if (reference_object_1.ReferenceObjectCodec.is(requestBody)) {
            ...
            const serialized = pipeable_1.pipe(request_body_object_1.serializeRequestBodyObject(from, requestBody), fp_ts_1.either.map(serialized => serialized_type_1.getSerializedOptionalType(required, serialized)));
            if (Either_1.isLeft(serialized)) {
                return serialized;
            }
...
exports.serializeOperationObject = reader_utils_1.combineReader(Reader_1.ask(), exports.getParameters, (e, getParameters) => (pattern, method, from, kind, operation, pathItem) => {
    ...
    const serializedResponses = responses_object_1.serializeResponsesObject(from)(operation.responses);
    ...
    return either_utils_1.combineEither(parameters, serializedResponses, client_1.clientRef, (parameters, serializedResponses, clientRef) => {
        ...
        const bodyIO = pipeable_1.pipe(parameters.serializedBodyParameter, fp_ts_1.option.map(body => `function withExplicitNulls(codec , input) { if (typeof input === 'object' && input !== null && !Array.isArray(input)) { return input; } const newObj = { ...input };const props = codec.props;for (const key in props) {if (!(key in newObj)) {newObj[key] = none;}} return newObj;};const valueWithNulls = withExplicitNulls(${body.io}, parameters.body);const body = ${body.io}.encode(valueWithNulls);`), fp_ts_1.option.getOrElse(() => ''));
